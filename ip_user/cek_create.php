...
$query = "SELECT ip_address FROM tb_ip";
$result = mysqli_query($con, $query);

if (mysqli_num_rows($result) > 0) {
    // Alamat IP sudah ada dalam tabel
    echo "<script>alert('IP sudah ada');
    window.location='".base_url('ip_user/create.php')."';</script>";
} else {
...
$query = mysqli_query($con, "insert into tb_ip (ip_address,segment,status,keterangan_ip,id_user) value('$ip','$segment','$status','$keterangan','$id_user')");
if ($query) {
